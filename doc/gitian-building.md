...
*Setup instructions for a gitian build of Sapphire using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Sapphire
...
- [Building Sapphire](#building-sap)
...
Clone the git repositories for sap and gitian and then checkout the sap version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/sap-crypto/sap.git
cd sap
...
Follow the instructions in [doc/release-process.md](release-process.md) in the sap repository
...
To build Sapphire (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the sap repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sap/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/sap-crypto/sap
...
For example:
```bash
URL=https://github.com/crowning-/sap.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit sap=${COMMIT} --url sap=${URL} ../sap/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sap=${COMMIT} --url sap=${URL} ../sap/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sap=${COMMIT} --url sap=${URL} ../sap/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sap-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sap-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/sap-build.assert
```
...
[sap/gitian.sigs](https://github.com/sap-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
