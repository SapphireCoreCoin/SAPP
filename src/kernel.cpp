...
bool Stake(CStakeInput* stakeInput, unsigned int nBits, unsigned int nTimeBlockFrom, unsigned int& nTimeTx, uint256& hashProofOfStake)
{
    ...
    int nHashDrift = 60; // Decrease the number of wasted CPU cycles
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZSapStake(spend));
    } else {
        ...
        CSapStake* sapInput = new CSapStake();
        sapInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(sapInput);
    }
