...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SAPP);
    ...
    fEnableZeromint = Params().ZeroCoinEnabled() && settings.value("fZeromintEnable").toBool() ? true : false;
    ...
    if (!settings.contains("nAnonymizeSapphireAmount"))
        settings.setValue("nAnonymizeSapphireAmount", 1000);

    nAnonymizeSapphireAmount = settings.value("nAnonymizeSapphireAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSapphireAmount"))
        SoftSetArg("-anonymizesapamount", settings.value("nAnonymizeSapphireAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in sap.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSapAmount:
            return QVariant(nAnonymizeSapphireAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case Language:
            ...
            break;
        case ZeromintEnable:
            fEnableZeromint = Params().ZeroCoinEnabled() && value.toBool() ? true : false;
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeSapAmount:
            nAnonymizeSapphireAmount = value.toInt();
            settings.setValue("nAnonymizeSapphireAmount", nAnonymizeSapphireAmount);
            emit anonymizeSapAmountChanged(nAnonymizeSapphireAmount);
            break;
        case CoinControlFeatures:
