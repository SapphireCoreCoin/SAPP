...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::SAPP)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sSAPPercentage, QString& szSAPPercentage)
{
    int nPrecision = 2;
    ...
    szSAPPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sSAPPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& earnings, const CAmount& masternodeEarnings, const CAmount& stakeEarnings)
{
    ...
    CAmount sapAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // SAP Watch-Only Balance
    ...
    // Combined balances
    CAmount availableTotalBalance = sapAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // SAP labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, sapAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    QString automintHelp = tr("Current percentage of zSAP.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", Params().ZeroCoinEnabled());
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in sap.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in sap.conf");
    }
    ...
    // SAP Available
    bool showSAPAvailable = settingShowAllBalances || sapAvailableBalance != nTotalBalance;
    bool showWatchOnlySAPAvailable = showSAPAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showSAPAvailable || showWatchOnlySAPAvailable);
    ui->labelBalance->setVisible(showSAPAvailable || showWatchOnlySAPAvailable);
    ui->labelWatchAvailable->setVisible(showWatchOnlySAPAvailable && showWatchOnly);

    // SAP Pending
    bool showSAPPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlySAPPending = showSAPPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showSAPPending || showWatchOnlySAPPending);
    ui->labelUnconfirmed->setVisible(showSAPPending || showWatchOnlySAPPending);
    ui->labelWatchPending->setVisible(showWatchOnlySAPPending && showWatchOnly);

    // SAP Immature
    bool showSAPImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = showSAPImmature || watchImmatureBalance != 0;
    ui->labelImmatureText->setVisible(showSAPImmature || showWatchOnlyImmature);
    ui->labelImmature->setVisible(showSAPImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

    // SAP Locked
    bool showSAPLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlySAPLocked = showSAPLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showSAPLocked || showWatchOnlySAPLocked);
    ui->labelLockedBalance->setVisible(showSAPLocked || showWatchOnlySAPLocked);
    ui->labelWatchLocked->setVisible(showWatchOnlySAPLocked && showWatchOnly);
    ...
    ui->labelSAPPercent->setVisible(false);
    ui->labelzSAPPercent->setVisible(false);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("SAP")
    updateDisplayUnit();
