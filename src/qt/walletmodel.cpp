...
void WalletModel::checkBalanceChanged()
{
    ...
    if (cachedBalance != newBalance || cachedUnconfirmedBalance != newUnconfirmedBalance || cachedImmatureBalance != newImmatureBalance ||
        ...
        cachedTxLocks != nCompleteTXLocks || cachedEarnings != newEarnings || cachedMasternodeEarnings != newMasternodeEarnings || cachedStakeEarnings != newStakeEarnings) {
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered sap address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SAPP.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal sap:URI (sap:XyZ...?message=example)
            {
...
static void NotifyzSAPReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSAPReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSAPReset.connect(boost::bind(NotifyzSAPReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSAPReset.disconnect(boost::bind(NotifyzSAPReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zsapTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
