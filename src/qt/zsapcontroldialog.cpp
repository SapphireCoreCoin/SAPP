...
#include "ui_zsapcontroldialog.h"
...
std::set<std::string> ZSapControlDialog::setSelectedMints;
std::set<CMintMeta> ZSapControlDialog::setMints;

bool CZSapControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZSapControlDialog::COLUMN_DENOMINATION || column == ZSapControlDialog::COLUMN_VERSION || column == ZSapControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZSapControlDialog::ZSapControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSapControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSapControlDialog::~ZSapControlDialog()
{
    delete ui;
}

void ZSapControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSapControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZSapControlWidgetItem* itemDenom(new CZSapControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZSapControlWidgetItem *itemMint = new CZSapControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zSAP seed used to mint this zSAP is not the same as currently hold in the wallet";
            else
...
void ZSapControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSapControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSap_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSapControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSapControlDialog::GetSelectedMints()
{
...
void ZSapControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
