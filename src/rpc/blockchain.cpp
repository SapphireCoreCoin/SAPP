...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zsapObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zsapObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zsapObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "  \"zSAPsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zSAP denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zSAP denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zSAP denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zSAP denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zSAP denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zSAP denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zSAP denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zSAP denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zSAP denominations\n"
            "  }\n"
            "}\n"
...
void RPCNotifyBlockChange(const uint256 hashBlock)
{
    ...
    if(pindex) {
        std::lock_guard<boost::mutex> lock(cs_blockchange);
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in sap\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of sap addresses\n"
            "     \"sapaddress\"   	 	(string) sap address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, 100, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZSAP_FAILED_ACCUMULATOR_INITIALIZATION);
