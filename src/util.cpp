...
int nAnonymizeSapphireAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "sap" is a composite category enabling all SAP-related debug output
            if (ptrCategory->count(string("sap"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\SAP
// Windows >= Vista: C:\Users\Username\AppData\Roaming\SAP
// Mac: ~/Library/Application Support/SAP
// Unix: ~/.sap
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SAP";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "sap.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty sap.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override sap.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "sapd.pid"));
