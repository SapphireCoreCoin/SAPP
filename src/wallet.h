...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 SAP at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSAP send
enum ZerocoinSpendStatus {
    ZSAP_SPEND_OKAY = 0,                            // No error
    ZSAP_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSAP_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSAP_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSAP_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSAP_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSAP_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSAP_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSAP_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSAP_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSAP_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSAP_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSAP_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSAP_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSAP_SPENT_USED_ZSAP = 14,                      // Coin has already been spend
    ZSAP_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSAP_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZSapBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZSAPOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzsapAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzSAPTracker> zsapTracker;
    ...
    void setZWallet(CzSAPWallet* zwallet)
    {
        zwalletMain = zwallet;
        zsapTracker = std::unique_ptr<CzSAPTracker>(new CzSAPTracker(strWalletFile));
    }

    CzSAPWallet* getZWallet() { return zwalletMain; }
    ...
    void setZSapAutoBackups(bool fEnabled)
    {
    ...
    /** zSAP reset */
    boost::signals2::signal<void()> NotifyzSAPReset;
