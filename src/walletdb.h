...
public:
    ...
    bool WriteZSAPSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSAPSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSAPSeed_deprecated(uint256& seed);
    bool EraseZSAPSeed();
    bool EraseZSAPSeed_deprecated();

    bool WriteZSAPCount(const uint32_t& nCount);
    bool ReadZSAPCount(uint32_t& nCount);
