...
CzSAPTracker::CzSAPTracker(std::string strWalletFile)
{
...
CzSAPTracker::~CzSAPTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSAPTracker::Init()
{
...
bool CzSAPTracker::Archive(CMintMeta& meta)
{
...
bool CzSAPTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSAPTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSAPTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSAPTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSAPTracker::GetSerialHashes()
{
...
CAmount CzSAPTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSAPTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSAPTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSAPTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSAPTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSAPTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSAPTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSAPTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSAPTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSAPTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSAPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSAPWallet* zSAPWallet)
{
    bool iszSAPWalletInitialized = (NULL != zSAPWallet);
    CMintMeta meta;
    ...
    if (! iszSAPWalletInitialized)
        zSAPWallet = new CzSAPWallet(strWalletFile);
    meta.isSeedCorrect = zSAPWallet->CheckSeed(dMint);
    if (! iszSAPWalletInitialized)
        delete zSAPWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSAPTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSAPTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSAPTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSAPTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSAPTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSAPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSAPWallet* zSAPWallet = new CzSAPWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zSAPWallet);
        }
        delete zSAPWallet;
        LogPrint("zero", "%s: added %d dzsap from DB\n", __func__, listDeterministicDB.size());
    }
