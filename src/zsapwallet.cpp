...
CzSAPWallet::CzSAPWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsap seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSAPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSAPSeed_deprecated()) {
                    LogPrintf("%s: Updated zSAP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsap seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsap wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSAPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSAPCount(nCountLastUsed);
    else if (!walletdb.ReadZSAPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSAPWallet::Lock()
{
    seedMaster = 0;
}

void CzSAPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSAPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSAP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSAPWallet::LoadMintPoolFromDB()
{
...
void CzSAPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSAPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSAPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsapTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSAPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSAP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsapTracker which also adds to database
    pwalletMain->zsapTracker->Add(dMint, true);
...
void CzSAPWallet::SeedToZSAP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSAPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSAPWallet::GenerateDeterministicZSAP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSAPWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSAPWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSAPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
